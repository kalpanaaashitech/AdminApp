...
$id = $_GET['id'];
	...
	$sql3 = "SELECT m.id, c.comment_id, c.user_id, m.post_id, m.comments, u.first_name, u.last_name
	         FROM `tbl_what_post_comments` AS m 
			 JOIN `tbl_what_post_comments_user` AS c ON m.id = c.comment_id 
			 JOIN `tbl_users` AS u ON c.user_id = u.id 
        ...
        while ($com_fet = $com_qry->fetch_assoc())
        {
            $row_array['comments'][] = array(
					'id' => $com_fet['id'],
					'post_id' => $com_fet['post_id'],
					'comments' => $com_fet['comments'],
					'user_name' => $com_fet['first_name']." ".$com_fet['last_name']
            );
		}
		...
		$sql5 = "SELECT * FROM tbl_what_post 
		...
		$sql6 = "SELECT * FROM tbl_what_post 
		WHERE stories_id = $post_id 
			AND post_type ='5'  
		ORDER BY id desc "; 
