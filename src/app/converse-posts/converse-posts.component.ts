...
@Component({
  selector: 'app-converse-posts',
  templateUrl: './converse-posts.component.html',
  styleUrls: ['./converse-posts.component.css']
})
export class ConversePostsComponent implements OnInit {
  ...
  constructor(private dialogsService: DialogsService, private router: Router, private userService: UserService,  private route: ActivatedRoute) {
    window.scrollTo(0, 0);
    this.date = new(Date);
    this.userService.getConversePosts("0").subscribe( (posts) => { this.posts = posts, this.continues = posts[0].continues });
   }
   ...
   let max = document.documentElement.scrollHeight;
    if(pos == max && this.continues != '0')   {
       ...
       this.userService.getConversePosts(this.limitnew).subscribe( 
