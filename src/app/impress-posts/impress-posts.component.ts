...
@Component({
  selector: 'app-impress-posts',
  templateUrl: './impress-posts.component.html',
  styleUrls: ['./impress-posts.component.css']
})
export class ImpressPostsComponent implements OnInit {
  ...
  constructor(private dialogsService: DialogsService, private router: Router, private userService: UserService,  private route: ActivatedRoute) {
    window.scrollTo(0, 0);
    this.date = new(Date);
    this.userService.getImpressPosts("0").subscribe( (posts) => { this.posts = posts, this.continues = posts[0].continues });
   }
   ...
   let max = document.documentElement.scrollHeight;
    if(pos == max && this.continues != '0')   {
       ...
       this.userService.getImpressPosts(this.limitnew).subscribe( 
