...
@Component({
  selector: 'app-travel-posts',
  templateUrl: './travel-posts.component.html',
  styleUrls: ['./travel-posts.component.css']
})
export class TravelPostsComponent implements OnInit {
  ...
  constructor(private dialogsService: DialogsService, private router: Router, private userService: UserService,  private route: ActivatedRoute) {
    window.scrollTo(0, 0);
    this.date = new(Date);
    this.userService.getMustdosPosts("0").subscribe( (posts) => { this.posts = posts, this.continues = posts[0].continues });
   ...
   let max = document.documentElement.scrollHeight;
    if(pos == max && this.continues != '0')   {
       ...
       this.userService.getMustdosPosts(this.limitnew).subscribe( 
