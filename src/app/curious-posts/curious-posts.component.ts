...
@Component({
  selector: 'app-curious-posts',
  templateUrl: './curious-posts.component.html',
  styleUrls: ['./curious-posts.component.css']
})
export class CuriousPostsComponent implements OnInit {
  ...
  constructor(private dialogsService: DialogsService, private router: Router, private userService: UserService,  private route: ActivatedRoute) {
    window.scrollTo(0, 0);
    this.date = new(Date);
    this.userService.getCuriousPosts("0").subscribe( (posts) => { this.posts = posts, this.continues = posts[0].continues });
   }
   ...
   console.log(this.continues);
    ...
    if(pos == max && this.continues == '1')   {
       this.Continu = true;
       this.End = false;
       this.limit = "12";
       this.limitnew = +this.limitnew + +this.limit; 
       this.userService.getCuriousPosts(this.limitnew).subscribe( 
