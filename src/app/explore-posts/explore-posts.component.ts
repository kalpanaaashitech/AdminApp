...
@Component({
  ...
  styleUrls: ['./explore-posts.component.css']
})
export class ExplorePostsComponent implements OnInit {
  ...
  constructor(private dialogsService: DialogsService, private router: Router, private userService: UserService,  private route: ActivatedRoute) {
    window.scrollTo(0, 0);
    this.date = new(Date);
    this.userService.getExplorePosts("0").subscribe( (posts) => { this.posts = posts, this.continues = posts[0].continues });
   }
   ...
   let max = document.documentElement.scrollHeight;
    if(pos == max && this.continues != '0')   {
       ...
       this.userService.getExplorePosts(this.limitnew).subscribe( 
