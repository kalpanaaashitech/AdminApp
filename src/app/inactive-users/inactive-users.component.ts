...
export class InactiveUsersComponent implements OnInit {
  ...
  constructor(private dialogsService: DialogsService, public userService: UserService, public router: Router,  public route: ActivatedRoute) {
    window.scrollTo(0, 0);
    this.userService.getInActiveUsers().subscribe(  users => this.users = users);
   ...
   ActiveToUser(id:string,) : void {
    this.dialogsService.confirm('Confirm Dialog', 'Are you sure you want to Change Active this User ?')
    .subscribe((res) => {this.result = res},(error) => {console.log('error', error);}, 
    () => {  if(this.result == true) {
                this.user_id = id ;
                this.userService.reActiveUser(this.user_id).subscribe((status) => {this.block_status = status}, (error) => {console.log('error', error);}); 
